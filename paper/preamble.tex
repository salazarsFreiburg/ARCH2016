...
\title{Benchmark for Verification of Fault-Tolerant Clock Synchronization Algorithms\\
\normalsize (Benchmark Proposal)}
\author{%
  Sergiy Bogomolov$^{\ddagger}$
  \and
  Christian Herrera$^{\star,1}$
  \and
  Wilfried Steiner$^{\dagger}$}
\titlerunning{Benchmark for Verification of Fault-Tolerant Clock Sync. Algorithms}
